...
import { MdDescription, MdDelete, MdAccessTime, MdCancel, MdCloudUpload, MdInsertDriveFile, MdCheckCircle } from 'react-icons/md';
...
export default function AdminKnowledgeTab() {
    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
            <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 shadow-[0_4px_12px_rgba(0,24,72,0.04)]">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                            <MdCloudUpload className="w-6 h-6 text-on-surface" />
                    ...
                    <div className="shrink-0 flex flex-col items-start md:items-end">
                        <label className={`inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-semibold cursor-pointer transition-colors shadow-[0_4px_12px_rgba(0,24,72,0.04)] ${uploading ? 'opacity-70 pointer-events-none' : ''}`}>
                            {uploading ? (
                                <>
                        ...
                        <p className="text-xs text-outline mt-3 font-medium">Hỗ trợ PDF, TXT (Tối đa 10MB)</p>
                    </div>
                </div>
            </div>

            <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-[0_4px_12px_rgba(0,24,72,0.04)] overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-surface-container">
                    <h3 className="font-bold text-on-surface uppercase tracking-wide text-sm flex items-center gap-2">
                        <MdDescription className="w-4 h-4 text-on-surface-variant" />
                        Danh sách Tài liệu
                    </h3>
                    <span className="text-xs font-bold text-on-surface-variant bg-surface-container-lowest px-3 py-1 rounded-full border border-outline-variant shadow-[0_4px_12px_rgba(0,24,72,0.04)]">
                ...
                {loading && documents.length === 0 ? (
                    ...
                    <div className="p-12 text-center text-outline flex flex-col items-center">
                        <div className="w-16 h-16 bg-surface-container-low rounded-full flex items-center justify-center mb-4">
                            <MdInsertDriveFile className="w-8 h-8 text-slate-300" />
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm text-on-surface-variant">
                            <thead className="text-xs text-on-surface-variant uppercase bg-slate-50/50 border-b border-slate-100">
                                <tr>
                                ...
                                {documents.map(doc => (
                                    <tr key={doc.id} className="hover:bg-surface-container transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-900 flex items-center gap-3">
                                            <div className={`p-2 rounded-lg ${doc.file_type === 'application/pdf' ? 'bg-rose-50 text-rose-500' : 'bg-primary-container text-blue-500'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-xs font-bold px-2 py-1 rounded bg-surface-container-low text-on-surface-variant">
                                        ...
                                        <td className="px-6 py-4">
                                            {doc.status === 'COMPLETED' && (
                                                <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100">
                                                    <MdCheckCircle className="w-3.5 h-3.5" /> Đã ghi nhớ
                                                </span>
                                            )}
                                            {doc.status === 'PROCESSING' && (
                                                <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-100">
                                                    <MdAccessTime className="w-3.5 h-3.5 animate-spin" /> Đang học...
                                                </span>
                                            )}
                                            {doc.status === 'FAILED' && (
                                                <span className="inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full bg-rose-50 text-rose-600 border border-rose-100">
                                                    <MdCancel className="w-3.5 h-3.5" /> Lỗi đọc file
                                                </span>
                                            )}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button 
                                                ...
                                                className="p-2 text-outline hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
                                                title="Xóa tài liệu"
                                            >
