...
export default function ExercisesTable({ data }) {
    if (data.length === 0) {
        return <div className="p-8 text-center text-slate-500 font-medium">Không tìm thấy môn thể thao nào.</div>;
    }
            ...
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-slate-50/80 border-b border-slate-100">
                        <th className="px-6 py-4 text-xs font-extrabold text-slate-500 uppercase tracking-widest">Tên Môn Tập</th>
                        <th className="px-6 py-4 text-xs font-extrabold text-slate-500 uppercase tracking-widest text-center">Chỉ số MET</th>
                        ...
                        <tr key={ex.id} className="hover:bg-indigo-50/30 transition-colors group">
                            ...
                            <td className="px-6 py-4 text-right">
                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit className="w-4 h-4" /></button>
