    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 md:p-6 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200">
                ...
                {/* Header */}
                <div className="p-6 md:px-8 md:py-6 flex justify-between items-center bg-white z-10">
                    <div>
                        <h2 className="text-xl font-black text-slate-900 tracking-tight">Kho Món Ăn</h2>
                        <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-widest">Tìm kiếm và chọn món cho thực đơn</p>
                    </div>
                ...
                {/* Search & Stats */}
                <div className="px-6 md:px-8 pb-4 bg-white z-10 border-b border-slate-100">
                    ...
                    {dailyGoal && consumed && (
                        <div className="mt-4 p-5 bg-slate-50 rounded-2xl space-y-4 border border-slate-100">
                            {/* Calo */}
                            <div>
                                <div className="flex justify-between items-center mb-1.5">
                                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Ngân sách Calo</span>
                                    <span className="text-[11px] font-black text-slate-900">
                                        {Math.round(consumed.calories)} / {dailyGoal.calories} <span className="text-[10px] font-bold text-slate-400 uppercase">kcal</span>
                                    </span>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-1.5">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Tinh bột</span>
                                ...
                                <div>
                                    <div className="flex justify-between items-center mb-1.5">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Đạm</span>
                                        <span className="text-[10px] font-black text-slate-800">
                                            {Math.round(consumed.protein)} / {dailyGoal.protein}g
                                        </span>
                                    </div>
                                    <div className="w-full bg-slate-200 rounded-full h-1 overflow-hidden">
                                        <div className="bg-amber-500 h-full transition-all" style={{ width: `${Math.min(100, (consumed.protein / dailyGoal.protein) * 100)}%` }}></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-3">
                            {filteredDishes.map(dish => (
                                <div 
                                    key={dish.id} 
                                    className="p-4 rounded-2xl group border border-slate-100 hover:border-slate-300 hover:shadow-sm transition-all bg-white"
                                >
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-2.5 mb-1.5">
                                                <h4 className="font-bold text-slate-900 text-sm leading-tight">{dish.name}</h4>
                                                <div className="flex gap-1.5">
                                                    {dish.is_ai_generated && <span className="text-[9px] px-2 py-0.5 bg-violet-100 text-violet-700 rounded-full font-bold uppercase tracking-widest border border-violet-200">AI Gợi Ý</span>}
                                                    {dish.is_custom && !dish.is_ai_generated && <span className="text-[9px] px-2 py-0.5 bg-sky-100 text-sky-700 rounded-full font-bold uppercase tracking-widest border border-sky-200">Cá Nhân</span>}
                                                </div>
                                            </div>
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{dish.category}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            ...
                                            <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-2 py-1.5 focus-within:bg-white focus-within:border-slate-400 transition-all">
                                                ...
                                                />
                                                <span className="text-[10px] text-slate-400 font-bold uppercase pr-1">g</span>
                                            </div>

                                            {dish.is_custom && (
                                                <button 
                                                    onClick={() => handleDeleteCustom(dish.id)} 
                                                    className="p-2 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors border border-transparent"
                                            ...
                                            <button 
                                                ...
                                                className="px-4 py-2 bg-slate-900 text-white text-[11px] font-bold uppercase tracking-widest rounded-xl hover:bg-blue-700 transition-all active:scale-95 ml-1 shadow-md shadow-blue-600/20"
