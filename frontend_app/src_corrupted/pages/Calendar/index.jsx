...
export default function CalendarSchedule() {
    ...
    useEffect(() => {
        const fetchMonthly = async () => {
            ...
            try {
                ...
                console.error("L?i t?i l?ch tháng:", error);
            } finally {
    ...
    useEffect(() => {
        const fetchDaily = async () => {
            ...
            try {
                ...
                console.error("L?i t?i chi ti?t ngày:", error);
            } finally {
    ...
    const timelineEvents = [
        ...dailyMeals.map(m => ({
            id: `m-${m.id}`,
            time: m.meal_time,
            title: m.Dish ? m.Dish.name : 'Món an t? ch?n',
            desc: `N?p ${Math.round(m.total_calories_calculated)} kcal (Carb: ${Math.round(m.total_carbs_calculated)}g, Pro: ${Math.round(m.total_protein_calculated)}g)`,
            type: 'meal',
        ...
        ...dailyExercises.map(e => ({
            id: `e-${e.id}`,
            time: 'Th? thao',
            title: e.Exercise ? e.Exercise.name : 'Bài t?p t? do',
            desc: `Ð?t cháy ${Math.round(e.calories_burned)} kcal trong ${e.duration_minutes} phút`,
            type: 'exercise',
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight  flex items-center gap-3">
                        ...
                        L?ch Ho?t Ð?ng & Dinh Du?ng
                    </h1>
                    <p className="text-slate-500 font-medium mt-1">Theo dõi nh?t ký n?p và tiêu hao nang lu?ng m?i ngày.</p>
                </div>
            </div>
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 shadow-xl text-white relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-6 relative z-10">
                            <div>
                                ...
                                <h2 className="text-2xl font-bold">Thâm H?t Nang Lu?ng</h2>
                            </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-2 relative z-10">
                            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-2xl border border-white/5">
                                ...
                                <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">N?p vào</div>
                            ...
                            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-2xl border border-white/5">
                                ...
                                <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">TDEE / N?n</div>
                        ...
                        <p className="text-xs text-slate-400 font-medium mt-3 text-center relative z-10">
                            Thâm h?t = (TDEE + Tiêu hao) - N?p vào
                        </p>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex-1">
                        <h3 className="font-bold text-slate-800 text-lg mb-6 flex items-center gap-2">
                            <Clock className="text-blue-600 w-5 h-5" /> Chi ti?t ho?t d?ng
                        </h3>
                        ...
                        {isLoadingDay ? (
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className="text-slate-500 font-medium">Chua có d? li?u an u?ng/t?p luy?n trong ngày này.</p>
                            </div>
                        ) : (
