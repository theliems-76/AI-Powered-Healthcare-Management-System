...
export default function AIResults({ result, userRole }) {
   ...
   const handleDownloadPDF = useReactToPrint({
    ...
    onBeforePrint: () => {
        toast.info("Ðang m? c?a s? in PDF...", { autoClose: 2000 });
        return Promise.resolve(); 
    },
    onAfterPrint: () => {
        toast.success("X? lý hoàn t?t!");
    }
});
    ...
    const getRiskLabel = (score) => {
        if (score <= 33) return { label: 'R?i ro th?p', color: 'text-emerald-700', bg: 'bg-emerald-100', icon: '??' };
        if (score <= 66) return { label: 'Nguy co ti?n ti?u du?ng', color: 'text-amber-700', bg: 'bg-amber-100', icon: '??' };
        return { label: 'Báo d?ng d?', color: 'text-rose-700', bg: 'bg-rose-100', icon: '??' };
    };
            ...
            <div className="space-y-8">
                ...
                <div className="relative overflow-hidden bg-white p-8 md:p-10 rounded-[2rem] border border-slate-200 shadow-xl">
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
                        <div className="text-center md:text-left flex-1">
                            ...
                            <h2 className={`text-4xl md:text-5xl font-extrabold tracking-tighter mb-4  ${riskInfo.color.replace('text-', 'text-')}`}>
                                {riskInfo.label}
                            </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-rose-50 p-8 rounded-3xl border border-rose-100 shadow-sm">
                            <div className="flex items-center space-x-3 mb-6">
                                ...
                                <Tooltip text="Các y?u t? d?y % r?i ro m?c b?nh LÊN CAO.">
                                    <h3 className="text-xl font-extrabold text-rose-900">Y?u t? C?nh báo</h3>
                                </Tooltip>
                            </div>
                        ...
                        <div className="bg-emerald-50 p-8 rounded-3xl border border-emerald-100 shadow-sm">
                            <div className="flex items-center space-x-3 mb-6">
                                ...
                                <Tooltip text="Các thói quen t?t giúp KÉO GI?M r?i ro m?c b?nh.">
                                    <h3 className="text-xl font-extrabold text-emerald-900">Y?u t? B?o v?</h3>
                                </Tooltip>
                            </div>
                            <ul className="space-y-4">
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-xl">
                    <div className="flex items-center space-x-4 mb-8">
                        ...
                        <div>
                            <h3 className="text-2xl font-extrabold tracking-tight text-slate-800">Phác d? Ði?u tr? & Dinh du?ng</h3>
                    ...
                    <div className="mt-12 pt-8 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 print:hidden">
                        <div className="flex space-x-4 w-full md:w-auto">
                            ...
                            >
                                <Printer size={18}/> T?i PDF Báo Cáo
                            </button>
                            
                            {(userRole === 'DOCTOR' || userRole === 'ADMIN') && (
                                <button 
                                    onClick={() => toast.success("Ðã luu h? so và g?i thông báo cho b?nh nhân thành công!")}
                                ...
                                >
                                    <Send size={18}/> Luu & G?i B?nh nhân
                                </button>
                            )}
                        </div>
                    </div>
