...
export default function ExerciseSearchModal({ isOpen, onClose, onAddExercise, onEditExercise }) {
    ...
    const fetchExercises = async () => {
        ...
        try {
            ...
            console.error("L?i l?y bài t?p:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleClearAll = async () => {
        if (!window.confirm("C?NH BÁO: Thao tác này s? xóa toàn b? các bài t?p trong Kho cá nhân c?a b?n (Bao g?m c? AI g?i ý). B?n có ch?c ch?n?")) return;
        try {
            await api.delete('/exercises/clear-all');
            toast.success("Ðã làm s?ch kho d? li?u cá nhân!");
            fetchExercises(); 
        } catch (error) {
            toast.error("L?i khi d?n kho!");
        }
    };

    const handleDeleteCustom = async (exId) => {
        if (!window.confirm("B?n có ch?c mu?n xóa bài t?p này kh?i kho?")) return;
        try {
            await api.delete(`/exercises/custom/${exId}`);
            toast.success("Ðã xóa bài t?p!");
            setExercises(prev => prev.filter(ex => ex.id !== exId));
        } catch (error) {
            const msg = error.response?.data?.error || "L?i khi xóa!";
            toast.error(msg);
        }
    };
    ...
    const handleAdd = (exercise) => {
        ...
        if (!mins || mins <= 0) return toast.warning("Vui lòng nh?p s? phút t?p h?p l?!");
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Kho Bài T?p</h2>
                        <p className="text-xs text-slate-500 font-medium mt-0.5">Tìm ki?m và ch?n bài t?p cho ngày hôm nay</p>
                    </div>
                ...
                <div className="p-5 border-b border-slate-100 bg-slate-50/50">
                    <div className="relative">
                        ...
                        <input 
                            type="text" placeholder="Tìm Ch?y b?, Boi l?i, Yoga..." autoFocus
                    ...
                    <div className="flex justify-end mt-3">
                        ...
                        >
                            <Trash2 className="w-3.5 h-3.5" /> D?N S?CH KHO CÁ NHÂN
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-3 custom-scrollbar bg-white">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400">
                            ...
                            <p className="text-sm font-medium">Ðang t?i d? li?u...</p>
                        </div>
                    ) : filtered.length === 0 ? (
                        <div className="text-center py-12 text-slate-500">
                            <p className="text-sm font-medium">Không tìm th?y bài t?p nào.</p>
                            <p className="text-xs mt-1">Hãy th? tìm t? khóa khác ho?c t?o môn m?i.</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-2 mb-1">
                                                ...
                                                <div className="flex gap-1">
                                                    {ex.is_ai_generated && <span className="text-[9px] px-1.5 py-0.5 bg-amber-100 text-amber-800 rounded font-bold uppercase border border-amber-200">G?i ý AI</span>}
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Ch? s? MET: <span className="text-indigo-600">{ex.met_value}</span></span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            ...
                                            {(ex.is_ai_generated || ex.is_custom) && onEditExercise && (
                                                <button 
                                                    ...
                                                    title="Ch?nh s?a ch? s? MET"
                                                >
                                                    <Zap className="w-3.5 h-3.5" />
                                                    <span className="text-[11px] font-bold">Tinh ch?nh</span>
                                                </button>
                                            )}
